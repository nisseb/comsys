function [pol, apol] = spp(a,e, makemovie)
...
%             end
            ...
            B=sum(A')';
            A_col = sum(A2')';
            A_col2 = A_col .* ( abs((T(i,j) - T(:,j))) >= ang );
            
            %%%%% Task 3 %%%%%
            % Comment this away to use for task 2
            % reduces speed and accepts all angles if _collision_
            if (sum(A_col) > 1 && sum(A_col2) > 1)
                v(i) = 0.05;
                ang = 2*pi;
            end
